...
def dateSearchShortRange(cur,conn,startDate,endDate,accountName):   
    ...
    # Wait to load page
        sleep(0.0125*random.uniform(0.5, 1.5))
    ...
    for i in range(len(tweetsToGet)):
        ...
        sleep(0.001*random.uniform(0.5,1.5))
        ...
        atCollection=[]
        for a in ats:
            atCollection.append(a['href'].text)
        ...
        for div in contentTag[0].find_all(lambda tag: tag.name=="a"): 
            div.decompose()
        ...
        textFilteredNoHashtagsAts=contentTag[0].text
...
def searchLongRange(cur,conn,start=pd.to_datetime('2017-11-12'),end=pd.to_datetime('2017-11-13'),accountName="realDonaldTrump"):        
